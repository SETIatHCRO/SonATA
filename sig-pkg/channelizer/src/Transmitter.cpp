...
TransmitterTask::extractArgs()
{
	...
	//JR - June 18, 2014 - Added this for branching
	if(commandLineArgs->getBranchFreqMHZ() > 10.0)
	{
	  branchConnection = new Udp(std::string("branch-transmit"),
			  (sonata_lib::Unit) UnitTransmit);
	  if (!branchConnection)
	  	Fatal(ERR_CCC);
	  branchConnection->setAddress(commandLineArgs->getBranchAddr(), commandLineArgs->getBranchPort());
	  // set the buffer size
	  size_t branchSize = DEFAULT_SND_BUFSIZE;
	  branchConnection->setSndBufsize(branchSize);

	  branchMinFreq = commandLineArgs->getBranchFreqMHZ() - (0.8192/2.0);
	  branchMaxFreq = commandLineArgs->getBranchFreqMHZ() + (0.8192/2.0);

	  doBranching = true;
	  printFirstBranchPacket = true;

	  printf("Branching:\n");
	  printf("\tPort:     %d\n", (int)commandLineArgs->getBranchPort());
	  printf("\tFreq:     %f\n", (float32_t)commandLineArgs->getBranchFreqMHZ());
	  printf("\tMin Freq: %f\n", (float32_t)branchMinFreq);
	  printf("\tMax Freq: %f\n", (float32_t)branchMaxFreq);
	}
	else
	{
	  doBranching = false;
...
{
	Error err;
	//JR - June 18, 2014 - Added for branching
	bool doBanchingOrigValue;
	int  branchSendResult;
	doBanchingOrigValue = doBranching;
	...
	for (uint32_t i = 0; i < vector->size(); ++i) {
		...
		if(doBranching)
		{
			if(p.getHeader().freq >= branchMinFreq && p.getHeader().freq <= branchMaxFreq)
			{
				doBranching = false;
				ChannelDataPacket *packet = static_cast<ChannelDataPacket *>
					                        (p.getPacket());
				branchSendResult = branchConnection->send(packet, p.getPacketSize());

				if(printFirstBranchPacket == true)
				{
					printFirstBranchPacket = false;
					printf("BRANCHING, first packet sent, result = %d\n", branchSendResult);
				}
		...
		if(err)
		{
		  //JR - June 03. 2014 - Added extra debugging...
		  //Use ERR_SE.
		  LogError(ERR_SE, -1, "XCHAN SEND ERR: %d",  err);
		}
	}
	//JR - June 18, 2014 - Added for branching
	doBranching = doBanchingOrigValue;
