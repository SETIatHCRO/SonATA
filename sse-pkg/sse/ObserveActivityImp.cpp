...
{ 
	...
	findTargetsInPrimaryBeamFov(dbParametersForWorkThread_.getDbTargets(), 
			getMinDxSkyFreqMhz(),
			targetsToExclude);
		...
		vector<TargetInfo> & nominalTargetPositions)
{
			...
			it != preludeBeamsToUse.end(); ++it)
	{
		...
		lookUpTargetCoordinates(dbParameters_.getDbTargets(),
		...
		double * ra2000Rads, double * dec2000Rads)
{
			...
			ActParameters::PRIMARY_BEAM_POS_TARGET_ID)
	{
		targetId = actParameters_.getPrimaryTargetId();
		bool movingTarget(false);
		lookUpTargetCoordinates(dbParameters_.getDbTargets(), targetId,
		...
		bool *moving)
{
	...
	*dec2000Rads = pmAdjustedRaDec.dec.getRadian();
	...
	//JR - added debug statement to account for proper motion
	SseArchive::SystemLog() << "Proper Motion: targetId=" << targetId 
	       << "old Pos=" << raOrigRads << "," << decOrigRads
       	       << "New Pos=" << *ra2000Rads << "," << *dec2000Rads << endl;
