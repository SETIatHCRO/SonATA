...
public:
	...
	//JR - June 18, 2014 - Added to branch off a DX packet to another IP address
	IpAddress& getBranchAddr() { return (branch.addr); }
	int32_t getBranchPort() { return (branch.port); }
	float64_t getBranchFreqMHZ() { return (branchFreqMHZ); }
...
private:
	...
	//JR - June 18, 2014 - Added to branch off a DX packet to another IP address
	HostSpec branch;                                //Destination for a DX branch.
	float64_t branchFreqMHZ;                        //Frequency to branch off. If zero do not branch.
