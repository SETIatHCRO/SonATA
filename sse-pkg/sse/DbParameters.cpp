...
DbParametersInternal::DbParametersInternal():
   ...
   dbpastnamelist("dbpastnamelist", "comma separate db names", "db names to search for observed history", ""),
   db(NULL),
   connected(false),
   dbTargets(NULL),
   connectedTargets(false)
{}
...
void DbParameters::addParameters()
{
   ...
   addParam(internal_->nametargets);
   addParam(internal_->dbpastnamelist);
...
MYSQL* DbParameters::getDbTargets()
{
   if (!internal_->isConnectedTargets())
   {
      internal_->connectTargets();
   }
   return(internal_->dbTargets);
}
...
bool DbParametersInternal::isConnectedTargets() 
{
...
void DbParametersInternal::connectTargets()
{
	stringstream strm;

	dbTargets = mysql_init(NULL);
	...
	if (!mysql_real_connect(
				...
				user.getCurrent().c_str(),
				passwd.getCurrent().c_str(),
				nametargets.getCurrent().c_str(),
				port.getCurrent(),
				NULL,  // socket name
				0))    // flags
	{
		strm << "::connectTargets() MySQL error: " << mysql_error(dbTargets) << endl;
...
void DbParametersInternal::disconnectTargets()
{
	mysql_close(dbTargets);
...
void DbParametersInternal::resetTargets()
{
	connectedTargets = false;
	dbTargets = NULL;
}
...
const string DbParameters::getDbNameTargets() const
{
	return internal_->nametargets.getCurrent();
}
const std::list<string> DbParameters::getDbNameList() const
{
	std::list<string> list;

	std::stringstream ss(internal_->dbpastnamelist.getCurrent());
	std::string item;
...
const string DbParameters::getDbNameListAsString() const
{
	return internal_->dbpastnamelist.getCurrent();;
}
