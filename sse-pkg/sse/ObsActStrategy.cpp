...
void ObsActStrategy::configureCommensalCalTimer()
{
   ...
   calTargets_.loadTargetsFromDb(nssParameters_.db_->getDbTargets(), phaseCalCatalog);
...
// remove observed frequencies for the ObsRange
	...
	/*
       TargetId firstTargetId = nssParameters_.act_->getTargetIdForBeam(
		       shortestListBeamName);
	//orderedTargets_->retrieveObsHistFromDbForZxs(zxObservedRanges, firstTargetId);
	retrieveObsRangeFromDbForZxs(zxObservedRanges, firstTargetId);
   ...
   float64_t minAcceptableRemainingBandMhz)
{
   ...
   orderedTargets_ = new OrderedTargets(
      ...
      nssParameters_.db_->getDbTargets(),
      nssParameters_.db_->getDbNameList(),
      getVerboseLevel(),
...
// Get the observed min/max range for a target. The ZXs only.
// NOTE: This is copied from OrderedTargets::retrieveObsHistFromDbForZxs()
// but is used to avoid having to have a valid instance of OrderedTargets.
//  JR - March 16, 2014
/*
void ObsActStrategy::retrieveObsRangeFromDbForZxs(ObsRange &observedFreqs,
		TargetId target)
{
	//Check that there is s db defined.
	if (! nssParameters_.db_->useDb())
	{
		throw SseException( 
				"automatic target choice does not work when database is off\n",  
	...
	string methodName("retrieveObsRangeFromDbForZxs");
