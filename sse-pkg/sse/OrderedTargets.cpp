...
OrderedTargets::OrderedTargets(MYSQL* db,
   ...
   db_(db),
   dbTargets_(dbTargets),
   pastDbNames_(pastDbNames),
...
void OrderedTargets::retrieveObsHistFromDbForTargets(TargetMap & targetMap)
{
	...
	SseArchive::SystemLog() << "HERE in retrieveObsHistFromDbForTargets" << endl;
	SseArchive::SystemLog() << freqRangeLimitsMhz_.low_ << " MHz to " << freqRangeLimitsMhz_.high_ << " MHz" << endl;
	...
	*/
	...
	//SseArchive::SystemLog() << sqlstmt.str() << endl;
	...
	while (MYSQL_ROW row = mysql_fetch_row(query.getResultSet()))
	{
		...
		//SseArchive::SystemLog() << targetId << ": " << lowFreqMhz << " - " << highFreqMhz << endl;
	...
	VERBOSE2(getVerboseLevel(), "OrderedTargets::" << methodName
			<< " complete " << endl;);    
	SseArchive::SystemLog() << "Num of observed freq ranges:" << observedCount << endl;

	// Now also query the past databases
	for(std::list<string>::iterator list_iter = pastDbNames_.begin(); 
	    list_iter != pastDbNames_.end(); list_iter++)
	{
		SseArchive::SystemLog() << "PastDb:" << *list_iter << endl;

		string pastDbName = (string)(*list_iter);

		stringstream sqlstmt;
		sqlstmt << " select targetId, dxLowFreqMhz, dxHighFreqMhz"
			<< " from " << pastDbName << ".ActivityUnits"
			<< " where "
			<< " validObservation = 'Yes'"
			<< " and dxLowFreqMhz >= " << freqRangeLimitsMhz_.low_
			<< " and dxHighFreqMhz <= " << freqRangeLimitsMhz_.high_;
		...
		*/
		...
		while (MYSQL_ROW row = mysql_fetch_row(query.getResultSet()))
		{
			...
			//SseArchive::SystemLog() << pastDbName << ": " <<  targetId << ": " << lowFreqMhz << " - " << highFreqMhz << endl;
