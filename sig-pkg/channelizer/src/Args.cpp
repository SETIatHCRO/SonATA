...
static string usage = "channelizer [-?] [-A addr] [-a port] [-b] [-B bw] [-c channels] [-C channels] [-D decimation] [-d filter file] [-f file] [-h port] [-H host] [-i port] [-I host] [-j port] [-J host] [-M] [-m] [-N foldings] [-O oversampling] [-o] [-P polarization] [-S src] [-w workers] [-W worker thread first CPU]\n\
	...
	-Y freqMHz,ipAddress,port: branch off a DX packets for a frequency (defaults to nothing)\n\n";
...
Args::Args(): initBufs(false), localMode(false), logSseMsgs(false),
		...
		branch("0.0.0.0", 0), branchFreqMHZ(0.0)
...
{
	bool done = false;
	const char *optstring = "?bLMmnopVA:a:B:C:c:D:d:F:f:H:h:I:i:J:j:l:N:O:P:Q:R:S:s:t:T:w:W:Y:";
	...
	while (!done) {
		...
		case 'W':
			...
			break;
		case 'Y':
			memcpy(branch.addr, optarg, sizeof(branch.addr));
			branch.port = atoi(argv[optind]);
			branchFreqMHZ = atof(argv[optind+1]);
			break;
		default:
