...
ReceiverTask::routine()
{
	...
	bool done = false;
	bool first = true;
	//JR - Feb 05, 2014 - if get ETIMEDOUT, try one more time.
	int timedOutCount = 0;
	while (!done) {
		...
		if (err) {
			...
			case ETIMEDOUT:
				...
				if(err == ETIMEDOUT && timedOutCount == 0)
				{
					timedOutCount++;
					LogError(ERR_DCE, activity->getActivityId(), "ETIMEDOUT 1st time");
				}
				else
				{
					// socket has been closed
					// JR - Feb 04, 2014 - Added error number to the error message.
					// This error message makes its way back to the sse, in the 
					// control components log file.
					LogError(ERR_DCE, activity->getActivityId(), "Error=%d, recvfrom", err);
					pktList->free(pkt);
					kill();
				}
				break;
		...
		if (!testCancel()) {
			...
			//JR = Feb 04, 2014 
			// The original send() to the queue would immediately return an error
			// if the queue recv() is a little late. This to me is a race condition.
			// I changed it to give it 3 chances, waiting 500ms the second and third time.
			//Assert(!inQ->send(msg, 0));
			if(inQ->send(msg, 0))
				if(inQ->send(msg, 500))
					Assert(!inQ->send(msg, 500));
		}
		else {
			udp->terminate();
			pktList->free(pkt);
